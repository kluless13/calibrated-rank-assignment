...
**Data availability.** The underlying sequence data are publicly available: fish COI records from BOLD (Teleostei) and the time-calibrated phylogeny from the Fish Tree of Life (Rabosky et al., 2018; https://doi.org/10.1038/s41586-018-0273-1). All analysis code, the data-fetching and split-construction scripts (including the deduplication and leakage-audit procedure), run ledgers, figure scripts, and the source tables underlying the reported figures are openly available in a public repository (https://github.com/kluless13/calibrated-rank-assignment), from which the training and evaluation splits are fully reproducible. A versioned release will be archived with a Zenodo DOI, and per-record BOLD process-ID lists for each split will be provided, upon acceptance.

...
QUEUE_ROOT="${ROOT}/retrieval_dl_pairwise_tree_reranker"
QUEUE_LOG="${QUEUE_ROOT}/retrieval_dl_pairwise_tree_reranker.log"
QUEUE_PID="${QUEUE_ROOT}/retrieval_dl_pairwise_tree_reranker.pid"
TREE_FILE="data/phylo/actinopt_12k_treePL.tre"
ARM="coi_cnn_retrieval_hybrid_seed1301"
OUT="${RERANK_ROOT}/${ARM}_top50_blast_vsearch_tree10_pairwise"
mkdir -p "${QUEUE_ROOT}" "${RERANK_ROOT}"

if [[ -f "${QUEUE_PID}" ]] && ps -p "$(cat "${QUEUE_PID}")" >/dev/null 2>&1; then
  echo "Paper 1 retrieval-DL pairwise tree reranker is already running: PID $(cat "${QUEUE_PID}")"
  exit 0
fi
...
(
  ...
  echo "[$(date -Iseconds)] Paper 1 retrieval-DL pairwise tree reranker starting"
  "${PY}" - <<'PY'
import torch
...
PY
  ...
  if [[ -f "${OUT}/candidate_reranker_manifest.json" ]]; then
    echo "[$(date -Iseconds)] SKIP pairwise reranker arm=${ARM}"
    exit 0
  fi
  ...
  "${PY}" -u scripts/edna/train_paper1_candidate_listwise_reranker.py \
    ...
    --hidden-dim 128 \
    --dropout 0.1 \
    --log-file "${QUEUE_ROOT}/${ARM}_tree10_pairwise.log"

  echo "[$(date -Iseconds)] Paper 1 retrieval-DL pairwise tree reranker complete"
) > "${QUEUE_LOG}" 2>&1 &

echo $! > "${QUEUE_PID}"
echo "launched Paper 1 retrieval-DL pairwise tree reranker PID $(cat "${QUEUE_PID}")"

#!/usr/bin/env python3
"""Summarize MarkerMirror + VSEARCH same-marker union candidate support."""
...
def main() -> None:
    ...
    vs = pd.read_csv(args.vsearch_support)
    top_k = int(args.top_k)
    keep = ["query_id", *[f"top{top_k}_{rank}_hit" for rank in RANKS], "candidate_count"]
    vs = vs[[column for column in keep if column in vs.columns]].copy()
    merged = mm.merge(vs, on="query_id", how="left", suffixes=("", "_vsearch"))
    for rank in RANKS:
        merged[f"vsearch_top{top_k}_{rank}_hit"] = merged[f"top{top_k}_{rank}_hit"].map(truthy)
        merged[f"marker_mirror_top{top_k}_{rank}_hit"] = merged[f"marker_mirror_top50_{rank}_hit"].map(truthy)
        merged[f"union_vsearch_top{top_k}_{rank}_hit"] = (
            merged[f"marker_mirror_top{top_k}_{rank}_hit"] | merged[f"vsearch_top{top_k}_{rank}_hit"]
        )
    ...
    summary = pd.DataFrame(
        [
            ...
            support_summary(merged, "same_marker_12s_vsearch_global", f"vsearch_top{top_k}", top_k, total),
            support_summary(merged, "union_marker_mirror_plus_vsearch", f"union_vsearch_top{top_k}", top_k, total),
        ]
    )
    per_query_path = args.output_dir / "marker_mirror_union_vsearch_candidate_support_per_query.csv"
    summary_path = args.output_dir / "marker_mirror_union_vsearch_candidate_support_summary.csv"
    manifest_path = args.output_dir / "marker_mirror_union_vsearch_candidate_support_manifest.json"
    ...
    manifest = {
        ...
        "inputs": {
            ...
            "vsearch_support": rel(args.vsearch_support),
        },
        "outputs": {
        ...
        "top_k": top_k,
        "n_queries": int(total),
        "claim_boundary": "Union support summary over MarkerMirror 12S->16S and VSEARCH global same-marker candidates.",
    }

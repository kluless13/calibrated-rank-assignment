#!/usr/bin/env python3
"""Diagnostic policy using VSEARCH marker-resolvability cluster features.

Exp 129 replaced the rare-kmer marker-resolvability proxy with VSEARCH
near-exact clustering. This script asks whether production-available VSEARCH
cluster features can improve MarkerMirror/BLASTN/VSEARCH rank policy transfer.

Important boundary: the Exp 129 oracle-supported columns use known query labels
and are therefore not used as model features. This diagnostic uses only cluster
features that could exist for unlabeled input, such as whether a query cluster
contains reference records and the reference/query record counts in that
cluster.
"""
...
SCRIPT_NAME = "build_marker_mirror_vsearch_resolvability_policy_diagnostic"
ROOT = Path(".")
SOURCE_TABLE_DIR = ROOT / "results" / "paper1_phylo_calibrated_assignment" / "source_tables"
DEFAULT_VSEARCH_ROOT = (
    ROOT
    / "results"
    / "remote_runs"
    / "2026-06-04"
...
OUTPUT_DIR = (
    ...
    / "vsearch_resolvability_policy_diagnostic"
)
    ...
    return pd.DataFrame(
        {
            ...
            "query_sequence_length": pd.to_numeric(frame.get("seq_len"), errors="coerce"),
        }
    )


def load_identity_features(root: Path, identity: float, logger: ProgressLogger) -> pd.DataFrame:
    label = identity_label(identity)
    query_path = root / f"query_oracle_id{label}.csv"
    cluster_path = root / f"clusters_id{label}.csv"
    queries = read_csv(query_path, logger)
    clusters = read_csv(cluster_path, logger)
    merged = queries[
        ["processid", "cluster_id", "cluster_has_reference"]
    ].rename(columns={"processid": "query_id"})
    merged = merged.merge(
        clusters[
            [
                "cluster_id",
    ...
    prefix = f"vsearch_id{label}"
    out = pd.DataFrame(
        {
            "query_id": merged["query_id"].astype(str),
            f"{prefix}_cluster_has_reference": merged["cluster_has_reference"].astype(bool).astype(int),
            f"{prefix}_cluster_record_count": pd.to_numeric(
                merged["record_count"], errors="coerce"
            ),
            f"{prefix}_cluster_reference_record_count": pd.to_numeric(
                merged["reference_record_count"], errors="coerce"
            ),
            f"{prefix}_cluster_query_record_count": pd.to_numeric(
                merged["query_record_count"], errors="coerce"
            ),
        }
    )
    denom = out[f"{prefix}_cluster_record_count"].replace(0, np.nan)
    out[f"{prefix}_cluster_reference_fraction"] = (
        out[f"{prefix}_cluster_reference_record_count"] / denom
    )
    out[f"{prefix}_cluster_query_fraction"] = (
        out[f"{prefix}_cluster_query_record_count"] / denom
    )
    return out


def load_vsearch_features(root: Path, identities: list[float], logger: ProgressLogger) -> pd.DataFrame:
    features: pd.DataFrame | None = None
    for identity in identities:
        frame = load_identity_features(root, identity, logger)
        features = frame if features is None else features.merge(frame, on="query_id", how="outer")
    if features is None:
        return pd.DataFrame(columns=["query_id"])
    has_reference_cols = [
        column for column in features.columns if column.endswith("_cluster_has_reference")
    ]
    features["vsearch_resolvability_reference_identity_count"] = features[
        has_reference_cols
    ...
    for _, row in features.iterrows():
        found = [
            identity
            for identity in identities
            if int(row.get(f"vsearch_id{identity_label(identity)}_cluster_has_reference", 0) or 0)
        ]
        max_identity.append(max(found) if found else 0.0)
    features["vsearch_resolvability_max_identity_with_reference"] = max_identity
    logger.log(f"built VSEARCH feature table rows={len(features):,}")
    return features


def add_policy_features(
    policy_rows: pd.DataFrame,
    meta: pd.DataFrame,
    vsearch_features: pd.DataFrame,
) -> pd.DataFrame:
    ...
    rows = rows.merge(vsearch_features, on="query_id", how="left")
...
def run_nested(
    ...
    for repeat in range(repeats):
        ...
        for rank in ranks:
            ...
            x_train, x_cal, x_eval, encoded_cols = encode(train, cal, eval_, rank_numeric, categorical)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--vsearch-root", type=Path, default=DEFAULT_VSEARCH_ROOT)
...
def main() -> None:
    ...
    identities = [float(value.strip()) for value in args.identities.split(",") if value.strip()]
    ...
    logger.log(
        f"identities={identities} ranks={ranks} targets={targets} "
        f"repeats={args.repeats} model={args.model_type}"
    )

    meta = load_query_meta(args.query_table)
    logger.log(f"loaded query meta rows={len(meta):,}")
    vsearch_features = load_vsearch_features(args.vsearch_root, identities, logger)
    policies = read_csv(args.policy_rows, logger)
    rows = add_policy_features(policies, meta, vsearch_features)
    feature_rows_path = args.output_dir / "marker_mirror_vsearch_resolvability_policy_rows.csv.gz"
    vsearch_feature_path = args.output_dir / "marker_mirror_vsearch_resolvability_query_features.csv"
    rows.to_csv(feature_rows_path, index=False)
    vsearch_features.to_csv(vsearch_feature_path, index=False)
    ...
    prefix = "marker_mirror_vsearch_resolvability_policy_diagnostic"
    outputs = {
        ...
        "query_features": args.source_table_dir / f"{prefix}_query_features.csv",
    ...
    local_outputs = {
        ...
        "query_features": vsearch_feature_path,
    ...
    vsearch_features.to_csv(outputs["query_features"], index=False)
    ...
    manifest = {
        ...
        "inputs": {
            ...
            "vsearch_root": str(args.vsearch_root),
        },
        "source_table_outputs": {key: str(value) for key, value in outputs.items()},
        "local_outputs": {key: str(value) for key, value in local_outputs.items()},
        "model_type": args.model_type,
        "ranks": ranks,
        "targets": targets,
        ...
        "repeats": args.repeats,
        "feature_boundary": (
            "Uses production-available VSEARCH cluster features only. "
            "Exp129 oracle-supported columns are not used as model features."
        ),
        "claim_boundary": (
            "Diagnostic only. Tests whether VSEARCH marker-resolvability "
            "features stabilize rank policy transfer; does not enable "
            "family/genus/species calls."
        ),
    }
